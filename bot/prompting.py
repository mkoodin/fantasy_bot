...
CORE_DIRECTIVE = (
    ...
    "BE WILLING TO BE NON-CONSENSUS. Agreeing with the rankings is worth "
    "nothing — anyone can read them, and a player priced correctly offers no "
    "edge. Where the underlying evidence contradicts the consensus, SAY SO and "
    "back yourself: state what the consensus is, state your call, and state "
    "the specific evidence the market has not priced yet. The bar is evidence, "
    "not confidence — participation data, a depth-chart move, a role change, "
    "an injury the rankings pre-date. Absent that, defer to the consensus and "
    "say you are deferring. Never split the difference to sound safe: a "
    "hedged answer that commits to nothing is worse than either call, because "
    "it cannot be acted on or judged afterwards. "
...
DRAFT_DIRECTIVE = (
    ...
    "that consensus — then depart from it where you have reason to, saying "
    "plainly that you are and why. Add edge: flag MISPRICED ADP — undervalued players "
