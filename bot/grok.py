...
def _news_instructions(team_context: str) -> str:
    ...
    return (
        ...
        "that changes whether I stash or drop him; (4) a suspension, holdout "
        "or trade that opens a job. "
        "\n\nDo NOT issue start/sit verdicts. You are reading a few hours of "
        "news; the lineup call is made elsewhere against every player's "
        "projection for this week, the matchup and the whole roster at once, "
        "and a verdict from here would contradict it on thinner evidence. "
        "Where news changes the lineup calculus, say what changed and whose "
        "role it affects, then say to confirm with /startsit — state the "
        "implication, not the decision. "
